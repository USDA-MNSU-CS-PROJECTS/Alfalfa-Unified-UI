...
The dashboard also re-checks each tool automatically every 10 seconds, so a tool that comes online will switch from **Offline** to **Online** on its own — no page reload needed. To change the interval, set the `STATUS_REFRESH_SECONDS` environment variable before launching the Unified UI:

```cmd
set STATUS_REFRESH_SECONDS=5
set PORT=7863
python app.py
```
